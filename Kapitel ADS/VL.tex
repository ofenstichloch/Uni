...
Teil a): Suchpfad $P_k$\\
Seien $P_k'$ und $P_k''$ eine Zerlegung von $P_k$: $P_k' = \{v\in P_k | key(v)<x_k\}$, $P_k'' = \{v\in P_k | key(v) > x_k\}$\\
$w\in P_k''$ war zum Zeitpunkt seiner Einfügung der kleinste aller Knoten mit Schlüssel $> x_k$, $v\in P_k$ der größte $<x_k$\\
Wir zeigen $E(|P_k'|) = H_k$, sowie $E(|P_k''|) = H_{n-k+1}$, daraus folgt a).\\
Zur Abschätzung von $E(|P_k'|)$ definieren wir ein Spiel:\\
Spiel A: Ziehe zufällige Schlüssel aus S und zähle wie oft ein Schlüssel kleiner als $x_k$ gezogen wird, der maximal ist. Das Spiel endet, sobald $x_k$ gezogen wird.\\
